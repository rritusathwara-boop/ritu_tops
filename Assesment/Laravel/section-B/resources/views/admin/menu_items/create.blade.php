    ...
    <div class="col-md-8">
        <div class="card shadow-sm border-0 rounded-3">
            <div class="card-header bg-success text-white py-3">
                <h3 class="h5 mb-0 fw-bold"><i class="bi bi-plus-circle me-2"></i>Add New Menu Item</h3>
            </div>
            <div class="card-body p-4">
                <form action="{{ route('admin.menu_items.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name') }}" placeholder="e.g. Margherita Pizza" required>
                        @error('name')
                            <span class="text-danger small mt-1 d-block">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description" id="description" rows="3" class="form-control @error('description') is-invalid @enderror" placeholder="Ingredients and details">{{ old('description') }}</textarea>
                        @error('description')
                            <span class="text-danger small mt-1 d-block">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6">
                            ...
                            <input type="number" step="0.01" min="0.01" name="price" id="price" class="form-control @error('price') is-invalid @enderror" value="{{ old('price') }}" placeholder="299.00" required>
                            @error('price')
                                <span class="text-danger small mt-1 d-block">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <button type="submit" class="btn btn-success fw-bold px-4">Save Menu Item</button>
                    </div>
                </form>
            </div>
        </div>
